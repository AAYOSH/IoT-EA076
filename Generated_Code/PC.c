...
**     Date/Time   : 2020-12-19, 16:24, # CodeGen: 1
**     Abstract    :
...
void ASerialLdd1_OnBlockReceived(LDD_TUserData *UserDataPtr)
{
  ...
  if (PC_InpLen < PC_INP_BUF_SIZE) {   /* Is number of bytes in the receive buffer lower than size of buffer? */
    ...
    if (PC_InpLen == PC_INP_BUF_SIZE) { /* Is number of bytes in the receive buffer equal to the size of buffer? */
  ...
  if ((Flags & ON_ERROR) != 0U) {      /* Is any error flag set? */
    ...
    if ((Flags & ON_FULL_RX) != 0U) {  /* Is OnTxChar flag set? */
      PC_OnFullRxBuf();                /* Invoke user event */
    }
  }
  (void)ASerialLdd1_ReceiveBlock(ASerialLdd1_DeviceDataPtr, &BufferRead, 1U); /* Receive one data byte */
}
